...
my $inp = q(drab tan bags contain 4 clear gold bags.
...
my $inp1 = q(shiny gold bags contain 2 dark red bags.
...
foreach (@rules) {
	$cnt++;
	if (/^(.+)\sbags\scontain\s(.+)\sbag[s]?\.$/) {
		my ($outer, $inner_list) = ($1, $2);
		$colors{$outer}++;
		#$count{$outer} = 0;
		my @inner = split(/ bag[s]?, /, $inner_list);
		foreach (@inner) {
			if (/^(\d+)\s(.+)$/) {
				my ($cnt, $col) = ($1, $2);
				$contains_directly{$outer}++ if ($col eq "shiny gold");
				push(@{$contains{$outer}}, "$col,$cnt");
				$within{$col}->{$outer}++;
				$colors{$col}++;
				
				#$count{$outer} += $cnt;
			}
			# 'no other' is also possible!
		}
	}
	else {
		die("Unknown rule:\n$_\n");
...
foreach (keys %{$within{"shiny gold"}}) {
...
foreach (@{$contains{"shiny gold"}}) {
	my ($col, $cnt) = split(/,/);
	if (defined($seen{$col})) {
		printf(STDERR "%s inside a shiny gold bag can contain another shiny gold\n", $_);
	}
}
...
{
	my ($col) = @_;
	if (defined($cached_count{$col})) { return $cached_count{$col}; }
	
	my $count = 0;
	foreach (@{$contains{$col}}) {
		my ($c, $n) = split(/,/);
		$count+= $n*(count_within($c) + 1); # Include the bags themselves!
	}
	$cached_count{$col} = $count;
	return $count;
}
