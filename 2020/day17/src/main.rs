...
fn parse3(inp: &str) -> Grid3 {
    ...
    let mut g3 = Grid3 {
        ystride: ystride,
        zstride: zstride,
        topleft: zstride * ISTEP + ystride * ISTEP + ISTEP,
        grid: vec![0; zstride as usize * zlen],
    };
        ...
        for i in 0..bytes.len() {
            if bytes[i] == b'#' {
                for dz in -1..=1 {
                    for dy in -1..=1 {
                        for dx in -1..=1 {
                            let adr = ((i + lstart) as i64 + dz * g3.zstride + dy * g3.ystride + dx)
                                as usize;
                            g3.grid[adr] += 1;
                        }
                    }
                }
                g3.grid[i + lstart] += 128 - 1; // Reverse the neighbor update of the cell itself
            }
        }
...
fn parse4(inp: &str) -> Grid4 {
    ...
    let mut g4 = Grid4 {
        ...
        zstride: zstride,
        wstride: wstride,
        topleft: wstride * ISTEP + zstride * ISTEP + ystride * ISTEP + ISTEP,
        grid: vec![0; wstride as usize * wlen],
    };
    ...
    for l in lines {
        ...
        for i in 0..bytes.len() {
            if bytes[i] == b'#' {
                ...
                g4.grid[i + lstart] += 128 - 1; // Reverse the neighbor update of the cell itself
            }
        }
        lstart += ystride as usize;
    }
...
fn gen1(g3: &Grid3) -> Grid3 {
    ...
    for i in 0..g3.grid.len() {
        ...
        if cell >= 128 {
            ...
            new3.grid[i] += 128 - 1; // Reverse the neighbor update of the cell itself
        }
    }
    new3
}
...
fn _checknb(g3: &Grid3) {
    for i in 0..g3.grid.len() {
        ...
        if curr != 0 {
            ...
            if (curr & 128) + nb != curr {
                println!(
                    "Found bad neighbor count for i = {i}: {} should have been {}",
                    curr, nb
                );
                panic!("");
            } else {
                print!("{}", if curr >= 128 { "#" } else { "." });
            }
        }
    }
    println!();
