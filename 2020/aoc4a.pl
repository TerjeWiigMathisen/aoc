...
my %ecl = ();
foreach ('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth') {
	$ecl{$_} = 1;
}
...
foreach (@pass) {
	...
	my @req = ('ecl','pid','eyr','hcl','byr','iyr','hgt');
	my $present = 0;
	foreach (@req) {
		$present += defined($pf{$_});
	}
	my $val = 0;
	if ($present >= 7) {
		if ($pf{'byr'} >= 1920 && $pf{'byr'} <= 2002) {
			if ($pf{'iyr'} >= 2010 && $pf{'iyr'} <= 2020) {
				if ($pf{'eyr'} >= 2020 && $pf{'eyr'} <= 2030) {
					if ($pf{'hgt'} =~ /^(\d+)(cm|in)$/) {
						my ($h, $m) = ($1, $2);
						if (($m eq 'cm' && $h >= 150 && $h <= 193) || ($m eq 'in' && $h >= 59 && $h <= 76)) {
							if ($pf{'hcl'} =~ /^#[\dabcdef]{6}$/) {
								if (defined($ecl{$pf{'ecl'}})) {
									if ($pf{'pid'} =~ /^\d{9}$/) {
										$valid++;
									}
								}
