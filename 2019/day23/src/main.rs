...
fn process(inp:String) -> i64
{
    ...
    loop {
        for i in 0..50 {
            if cpus[i].inputs.len() == 0 {cpus[i].inputs.push_back(-1);}
            cpus[i].run();
            while cpus[i].outputs.len() > 0 {
                let addr = cpus[i].outputs.pop_front().unwrap();
                let x = cpus[i].outputs.pop_front().unwrap();
                let y = cpus[i].outputs.pop_front().unwrap();
                if addr == 255 {
                    return y;
                }
                cpus[addr as usize].inputs.push_back(x);
                cpus[addr as usize].inputs.push_back(y);
            }
        }
    }
}
...
fn process2(inp:String) -> i64
{
    ...
    loop {
        ...
        if idle {
            cpus[0].inputs.push_back(nat_x);
            cpus[0].inputs.push_back(nat_y);
