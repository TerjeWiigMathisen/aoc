...
impl Cpu {
    ...
    fn mem(&self, ip:usize, mode:i64)->String
    {
    ...
    fn disasm(&self, startip:usize, instructions:usize)
    {
        let mut ip = startip;
...
fn process(inp:&str,par:&[i64]) -> i64
{
    ...
    for p in par {cpu.inputs.push_back(*p)}
//    cpu.disasm(0,10);
    cpu.run();
    cpu.outputs[0]
}
...
*/
    ...
    devtime.start();
    let part1 = process(&input,&[1]);
    let part2 = process(&input,&[2]);
    devtime.stop();
