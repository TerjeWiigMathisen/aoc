...
fn process(inp:String) -> (i32,i32)
{
    ...
    let oxy = discover(&mut cpu, &mut grid, 0, 0, 1);
    println!("Found Oxy @ ({:?}", oxy);
//    show(&grid);
    ...
    pq.push("0,0".to_owned(),1000);
    'outer: while pq.len() > 0 {
        let (key,pri) = pq.pop().unwrap();
        let xy:Vec<i32> = key.split(",").map(|k| {k.parse::<i32>().unwrap()}).collect();
//        show(&grid);
        // Try north, south, east, west:
        for (dx,dy, _dir) in [(0,-1,1),(0,1,2),(1,0,3),(-1,0,4)] {
            let nkey = format!("{},{}",xy[0]+dx,xy[1]+dy).to_owned();
            let p = grid.get(&nkey).unwrap();
            if *p > 0 {
                if *p == 2 {
                    p1 = (1001-pri);
                    break 'outer;
                ...
                if *p == 1 {
                    grid.insert(nkey.clone(), pri-1);
                    pq.push(nkey.clone(),pri-1);
                }
            }
        }
    }
    pq.clear();
    pq.push(format!("{ox},{oy}").to_owned(),1000);
    grid = grid_backup;
    while pq.len() > 0 {
        let (key,pri) = pq.pop().unwrap();
        let xy:Vec<i32> = key.split(",").map(|k| {k.parse::<i32>().unwrap()}).collect();
