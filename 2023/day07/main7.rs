...
fn hand_rank(hand:&[u8], rank:&[u8;256]) -> (usize, usize)
{
	...
	let mut wild_count:[u8;CARD_RANGE] = [0;CARD_RANGE];
    let mut rank1 = 0;
    let mut rank2 = 0;
	for h in 0..5 { 
        ...
        rank1 += r << (16-h*4);
        card_count[r] += 1; 
        let r = WILD[hand[h] as usize] as usize;
        rank2 += r << (16-h*4);
        wild_count[r] += 1; 
    }
...
fn win(ranked_hands:&mut Vec<usize>) -> usize
{
...
pub fn process(inp:&str) -> (usize, usize)
{
    ...
    while i < input.len() {
        let (r1,r2) = hand_rank(input[i..i+5]);
        i += 6;
        ...
        regular.push((r1 << 32) + bid);
        jokers_wild.push((r2 << 32) + bid);
    }
