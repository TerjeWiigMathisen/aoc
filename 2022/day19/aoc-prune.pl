...
{	
	...
	my $maxore = $oreore;
	$maxore = $clayore if ($clayore > $maxore);
	$maxore = $geoore if ($geoore > $maxore);
	$maxore = $obsore if ($obsore > $maxore);
	my $maxclay = $obsclay;
	my $maxobs = $geoobs;
...
#			$clay, $obs, $geo, @manu)) if ($t < 0);
		...
		my $prod = 0;
		# Try rgeo first, this always helps if there is any time left
		if ($robs) { # && $rore)
		...
		# Do we need more robs?
		if ($rclay && $robs < $maxobs) { # && $obs < $geoobs*2) { # && $rore) 
			my ($hore, $hclay, $hobs, $hgeo) = ($ore, $clay, $obs, $geo);
			for (my $tt = $t; $tt < $TLIMIT; $tt++) {
		...
		# Produce more rore?
		if ($rore < $maxore) { #if ($ore < $clayore + $geoore){
			my ($hore, $hclay, $hobs, $hgeo) = ($ore, $clay, $obs, $geo);
			for (my $tt = $t; $tt < $TLIMIT; $tt++) {
				my $ok = ($hore >= $oreore);
				$hore += $rore;
