...
sub enq
{
	...
	$pri = 1e12+$t*1e8-$rgeo*1e10-$geo*1e9-$robs*1e7-$obs*1e6-$rclay*1e5-$clay*1e4-$rore;
...
{	
	...
	while (my $item = $prio->pop()) {
		...
		printf("Time %2d: %s\n", $t, join(',', $rore, $rclay, $robs, $rgeo, $ore, 
			$clay, $obs, $geo, @manu)) if ($t < 12);
		
		if (scalar(@manu) < $firstgeo && $geo) {
			$firstgeo = scalar(@manu);
		}
		if (scalar(@manu) > $firstgeo+5 && $geo==0) {
			#next;
		}
