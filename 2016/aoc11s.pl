...
my $mv = 0;
my $el = 0;
@inp = ("sg,sm,pg,pm","tg,rg,rm,cg,cm","tm","","4");
			...
			if (collection_ok(@rest) && collection_ok(@pick)) {
				if ($ele < 3) { # && scalar(@pick) == 2) { # and substr($pick[0],0,1) eq 't') {
					...
					if (collection_ok(@next)) {
						...
						$f[$ele] = join(",",@rest);
						$f[$ele+1] = join(",",@next);
						my $q = join("\t", $moves+1,$dist-scalar(@pick)+1,join(";",$ele+1,@f));
#						printf("%s insert\n", $q);
						$pq->insert($q, $dist-scalar(@pick)+1);
					}
				}
...
#moves($mv,$el,'',@inp);
$part1 = movestack(0,@inp);
printf(STDERR "Part1 time = %f\n", time - $start);

$inp[0] .= ",eg,em,dg,dm";
$part2 = movestack(0,@inp);
