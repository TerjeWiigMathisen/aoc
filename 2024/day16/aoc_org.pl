...
while (my $key = $pq->pop()) {
	...
	die("Bad $x,$y,$dir,$cost") if ($x < 0 || $x >= length($map[0]) || $y < 0 || $y >= scalar(@map));
	...
	while (1) {
		last if (substr($map[$y],$x,1) eq '#');
		my ($lx,$ly) = ($x+$dx[($dir+3)&3],$y+$dy[($dir+3)&3]);
		my ($rx,$ry) = ($x+$dx[($dir+1)&3],$y+$dy[($dir+1)&3]);
		if (substr($map[$ly],$lx,1) ne '#' || substr($map[$ry],$rx,1) ne '#') {
			last;
		}
		$cost++;
