...
fn process(inp:String) -> (i64,i64)
{
    ...
    for line in inp.lines() {
        let mut secret = line.parse::<u32>().unwrap();
    ...
    let part2 = *sum.iter().max().unwrap() as i64;
...
fn main() {
    ...
    let bench_result = run_benchmark(30, |_| { process(input.clone()); }); bench_result.print_stats();
