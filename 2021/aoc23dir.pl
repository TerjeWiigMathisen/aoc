...
sub move
{
	...
	if (defined($visited{$key})) {
		$revisit++;
		if ($energy >= $visited{$key}) {
			$skip++;
			return;
		}
	}
		...
		for (my $x = 1; $x <= $COLS; $x++) {
			...
			if ($piece ge "A") {
				...
				if ($y > 1) { # Try to move to one of the top row spots:
					...
					# Is the final target available, i.e. only correct pieces?
					my $r = $ROWS;
					...
					if ($sx == $t && substr($inp[$r],$t,1) eq '.') { # Move direct
						my @m = @inp;
						substr($m[$y],$x,1) = ".";
						substr($m[$r],$t,1) = $piece;
						my $s = $sol.sprintf(" %s%d%d->%d%d",$piece,$x,$y,$t,$r);
						move($lvl+1, $energy + $e*(abs($t-$x)+($r+$y-2)), $s, @m);
					}
					elsif ($piece le 'B') {
						# try left first:
...
printf("Part1: %d\n", $part1);
printf("%d steps: %s\n", scalar(split(" ",$sol1)),$sol1);
printf("Part2: %1.0f\n", $part2);
printf("%d steps: %s\n", scalar(split(" ",$solution)),$solution);
